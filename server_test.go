...
func (codec *CodecEmulator) ReadHeader(req *Header) error {
	req.ServiceMethod = codec.serviceMethod
	req.Seq = 0
	return nil
}

func (codec *CodecEmulator) ReadBody(argv interface{}) error {
	if codec.args == nil {
...
func (codec *CodecEmulator) WriteHeaderBody(resp *Header, reply interface{}) error {
	if resp.Kind == Error {
		codec.err = errors.New(resp.Info.(string))
	} else {
...
func (WriteFailCodec) WriteHeaderBody(*Header, interface{}) error {
...
func benchmarkEndToEnd(dial func() (*Client, error), b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		//b.Run("ttt", func(pb *testing.B) {
		//fmt.Println("start PB--")
		reply := new(Reply)
		for pb.Next() {
			//for i:=0;i<b.N;i++ {
			err := client.Call("Arith.Add", args, reply)
			//fmt.Println(args,reply,err)
			if err != nil {
